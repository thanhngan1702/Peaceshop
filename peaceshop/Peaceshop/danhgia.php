        ...
        <div class="feedback">
            ...
            <div class="comment-box text-center">
                <div class="comment-area">
                    <textarea name="noidung" class="form-control" placeholder="Viết đánh giá của bạn ở đây..."
                        rows="4"></textarea>
                </div>
                <div style="text-align: left;margin-top: 10px;">
                    <label class="btn btn-danger">
                        <i class="fa fa-camera" style="font-size: 17px;"></i>
                        Chọn
                        ảnh
                        <input type="file" class="account-settings-fileinput form-control" name="image"
                            multiple="multiple" />
                    </label>
                </div>
                <div class="text-center mt-4">
                    <a href="taikhoancanhan.php?ac=thongtin" class="btn btn-info px-5">
                        <i class="fa fa-long-arrow-left ml-1"></i> Quay lại
                    </a>
                    <button class="btn btn-success send px-5" name="guidanhgia">
                        Gửi đánh giá
...
mysqli_select_db($connection, "peaceshop");
    if (isset($_POST['guidanhgia']) && isset($_COOKIE['account'])) {
            ...
            if(isset($_FILES['image'])){
                ...
                @move_uploaded_file($hinhanh_tmp, 'img/danhgia/' . $_FILES['image']['name']);
            }
            $sql = "insert into danhgia(user_name,id_pro,user_review,datetime,user_rating,image_danhgia) values ('$account','$id_pro','$noidung','$ngaythem','$sao','$hinhanh')";
            $query = mysqli_query( $connection, $sql);
            if ($query) {
                header('location:taikhoancanhan.php?ac=thongtin&tt=<div class="alert alert-success"><strong>Đánh giá thành công!</strong>.</div>');
            } else {
                header('location:taikhoancanhan.php?ac=thongtin&tt=<div class="alert alert-danger"><strong>Lỗi!</strong> Đã xảy ra vấn đề. Vui long thử lại sau.</div>');
            }
    }
?>